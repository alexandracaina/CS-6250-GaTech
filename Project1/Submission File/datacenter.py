...
class DataCenter(Topo):
	...
	#Add mid level switches to top level switch
	for mid_switch_num in range(1,fi+1):
		mid_switch_name = 'mls%d' %(mid_switch_num)
		mid_switch = self.addSwitch(mid_switch_name)
		
		self.addLink(tls, mid_switch, **swlinkConfig)
		
		#Add rack switches to each mid level switches
		for rack_switch_num in range(1, fi+1):
			rack_switch_name = 's%dx%d' %(mid_switch_num,rack_switch_num)
			rack_switch = self.addSwitch(rack_switch_name)

			self.addLink(mid_switch, rack_switch, **swlinkConfig)
				
        		#Add hosts for each rack switch
			for host_num in range(1, n+1):
				host_name= 'h%dx%dx%d' %(mid_switch_num, rack_switch_num, host_num)
				host= self.addHost(host_name, **hostConfig)

				self.addLink(host,rack_switch, **hostlinkConfig)
